...
public sealed class PatientAbsentAtConsultationProjectionHandler : IProjectionHandler
{
    public string EventName => nameof(PatientAbsentAtConsultation);
    ...
    {
        if (@event is not PatientAbsentAtConsultation evt)
            throw new ArgumentException($"Expected {nameof(PatientAbsentAtConsultation)}, got {@event.GetType().Name}");
        ...
        var idempotencyKey = $"patient-absent-consultation:{evt.QueueId}:{evt.Metadata.AggregateId}:{evt.Metadata.EventId}";
