...
import type { NextTurnView, AtencionStateView, RecentAttentionRecordView } from "@/services/api/types";
...
function buildAppointments(
  queueState: AtencionStateView | null,
...
export type AtencionAppointmentsState = {
...
export function useAtencionAsAppointments(
  serviceId: string,
  refreshInterval = 5_000,
): AtencionAppointmentsState {
  const { queueState, nextTurn, history, connectionState } = useAtencion(
    serviceId,
    refreshInterval,
  );
