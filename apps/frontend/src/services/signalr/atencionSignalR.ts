...
/** serviceId de la última conexión, usado para re-registrar tras reconexión. */
let lastServiceId: string | null = null;

export type AtencionHandlers = {
  onMonitor?: (payload: unknown) => void;
  onAtencionState?: (payload: unknown) => void;
 ...
 */
export async function connect(serviceId: string, handlers: AtencionHandlers = {}): Promise<HubConnection | null> {
  ...
  const newConn = new HubConnectionBuilder()
    .withUrl(url, {
      ...
      headers: { "X-Atencion-Id": serviceId },
    })
  ...
  newConn.on("AtencionStateUpdated", (payload: unknown) => handlers.onAtencionState?.(payload));
  // Legacy event name — backend emits both during transition; route to same handler
  newConn.on("QueueStateUpdated", (payload: unknown) => handlers.onAtencionState?.(payload));
  ...
  newConn.onreconnected((connectionId?: string | null) => {
    ...
    // Re-joining the group is handled by the backend on reconnect.
    // The explicit invoke is not needed and was causing a HubException.
    // void newConn.invoke("JoinAtencion", serviceId).catch(() => {
  ...
  try {
    ...
    // The 'X-Atencion-Id' header is used by the backend to handle group joining.
    // An explicit 'invoke' is not needed and was causing a HubException.
    // await newConn.invoke("JoinAtencion", serviceId);
    console.info(`SignalR: conectado al hub para la cola ${serviceId}`);
    handlers.onConnected?.();
  } catch (err) {
...
/** Devuelve el serviceId de la conexión activa, o null. */
