...
```bash
cd <ORCHESTRATOR_SOURCE>  # Ruta al repositorio que contiene el blueprint
...
```bash
sudo ln -s <ORCHESTRATOR_SOURCE>/scripts/orchestrator-init.sh /usr/local/bin/orchestrator-init
...
```bash
orchestrator-init -s <ORCHESTRATOR_SOURCE> --run-sync
```
