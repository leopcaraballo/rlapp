#!/bin/bash
# ================================================================
# sync.sh — Skill Reference Synchronizer
# ================================================================
...
AGENT_FILE="$REPO_ROOT/docs/decisions/SKILL_REGISTRY.md"
SKILLS_DIR="$REPO_ROOT/tools/automation/skills"
...
for skill_dir in "$SKILLS_DIR"/*/; do
    ...
    if [ ! -f "$skill_file" ]; then
        echo "   ⚠️  Skipping $skill_dir (no skill.md)"
        continue
    fi
    ...
    if [ -n "$name" ]; then
        ...
        echo "   ⚠️  ${skill_dir} has no 'name' in metadata"
    fi
done
...
echo ""
echo "🎉 Skill References synced in agent.md ($(echo -e "$TABLE_ROWS" | grep -c '|') skills)"
